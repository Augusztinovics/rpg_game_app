<template>
    <div>
        ...
        <p class="fs-4 border-bottom border-secondary"><span class="fw-bolder me-2">Vallás: </span><span>{{ Vallas.name }}</span><span><button v-if="Vallas.name !== ''" type="button" class="btn btn-outline-success btn-sm ms-2" data-bs-toggle="modal" data-bs-target="#vallasInfoModal">i</button><button v-if="Vallas.id != 'PAP' || Vallas.id != 'PAPLOVAG'" class="btn btn-outline-success btn-sm my-1 d-block" type="button" data-bs-toggle="modal" data-bs-target="#vallasModal">Változtat</button></span></p>

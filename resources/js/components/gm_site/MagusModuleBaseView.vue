<template>
    <div>
        <div class="p-4 border-bottom border-primary m-2 text-center">
            <button type="button" class="btn btn-success costum-btn">Új játék készítése</button>
        </div>
        ...
        <div class="table-responsive m-4">
            <table class="table table-striped">
                ...
                <tbody>
                    <tr v-for="gameModule, index in gameModules" :key="'GM' + index">
                        <td>{{ gameModule.id }}</td>
                        <td><a href="" class="btn btn-link">{{ gameModule.name }}</a></td>
                        <td>
                            <a href="" class="btn btn-outline-success m-1">Megnéz</a>
                            <button type="button" class="btn btn-success costum-btn m-1">Módosít</button>
                            <button type="button" class="btn btn-danger costum-btn m-1">Töröl</button>
                        </td>
                    </tr>
                </tbody>
            </table>
...
export default {
    ...
    methods: {
         fetchGameModules() {
            axios.get('gm/game-modules/MAGUS')
            .then(res => {
